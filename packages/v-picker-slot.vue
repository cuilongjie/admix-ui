...
export default {
    ...
    methods: {
        value2Translate (value) {
            let values = this.mutatingValues,
                // valueIndex = values.indexOf(value), // 组件使用时，如果用v-model外部值外部会跟着变，如果不用v-model绑定，比如p-position，要指定默认值，传入的对象和当前对象用这种方法无法判断为相等！-- Author by clj. on 2018.2.4
                valueIndex,
        ...
        initEvents () {
            ...
            // 阻止空白区域的滚动事件冒泡。 Author by clj. on 2017.4.5
