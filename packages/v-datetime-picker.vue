...
export default {
    ...
    props: {
        ...
        startDate: {
            ...
            default () {
                ...
                if (this.type === 'smart') sd = new Date(); // clj. on 2017.3.1
                return sd;
            }
        },
        endDate: {
            ...
            default () {
                ...
                if (this.type === 'smart') ed = new Date(cur.getFullYear(), cur.getMonth() + 2, cur.getDate()); // clj. on 2017.3.1
                return ed;
            }
        },
        startHour: {
    ...
    computed: {
        rims () {
            ...
            if (this.type === 'smart') { // 设定起止范围，日期默认从今天开始，两个月。 add by clj. on 2017.2.28
    ...
    watch: {
        ...
        currentDatesIdx (val) { // 显示为区间样式时，根据当前选择的时间切换currentValue的值。 mod by clj. on 2018.6.19
    ...
    mounted () {
        ...
        if (!this.value) {
            ...
            } else if (this.type.indexOf('smart') > -1) { // add by clj. on 2017.2.28
    ...
    methods: {
        ...
        /**
         * 格式化特殊日期              -- Author by clj. on 2017.2.28
         */
        getSpecialDate (str) {
        ...
        onChange (picker) {
            ...
            if (this.type === 'smart') { // smart模式时间校验。 add by clj. on 2017.3.31
                let std = new Date();
        ...
        pushSlots (slots, type, start, end) {
            ...
            if (this.type === 'smart') { // 日期自适应，两列时间固定宽度。 add by clj. on 2017.2.29
        ...
        handleExceededValue () {
            ...
            } else if (this.type === 'smart') { // add by clj. on 2017.2.28
        ...
        rimDetect (result, rim) {
            ...
            if (this.type === 'smart') { // add by clj. on 2017.3.1
        ...
        handleValueChange () {
            if (this.isRegion) { // mod by clj. on 2018.6.19
                if (this.currentDatesIdx) this.$set(this.currentDates, 1, {val: this.currentValue, lab: utils.formatTime(this.currentValue, 'yyyy-MM-dd')});
                else this.$set(this.currentDates, 0, {val: this.currentValue, lab: utils.formatTime(this.currentValue, 'yyyy-MM-dd')});
